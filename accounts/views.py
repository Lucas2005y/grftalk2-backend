...
class UserView(APIView):
    def get(self, request):
        #Update last_access    
        User.objects.filter(id=request.user.id).update(last_acess=now())
    ...
    def put(self, request):
        ...
        if avatar:
            content_type = avatar.content_type.split
            extension = avatar.name.split('.')[-1]
            
            #Validate avatar
            if not content_type == "image/png" and not content_type == "image/jpeg":
               raise ValidationError("Somente arquivos do tipo PNG e JPEG são permitidos")
